...
* **Role in MandirMusicBot:** Pyrogram acts as the central brain of the bot. It is responsible for handling all incoming messages, commands (e.g., `/play`, `/skip`), inline queries, and button callbacks.
* **Implementation:** Mostly configured in `MandirMusic/core/bot.py` and `MandirMusic/core/telegram.py`. 
...
* **Role in MandirMusicBot:** While Pyrogram handles the text and UI logic, PyTgCalls is entirely responsible for the audio processing and streaming. It connects the Assistant Account to the Telegram Voice Chat via WebRTC.
* **Implementation:** Managed centrally in `MandirMusic/core/calls.py`. It natively handles audio queues, pausing, resuming, and muting the stream directly inside the active voice chat.

### 3. yt-dlp & FFmpeg (Media Extraction & Processing)
* **yt-dlp:** We use `yt-dlp` (located in `MandirMusic/core/youtube.py`) to bypass the need to download entire videos. Instead, it extracts the direct high-quality audio stream URLs (like Opus/WebM) from YouTube and other platforms.
...
* **Role in MandirMusicBot:** To maintain the asynchronous nature of the bot, we use `Motor` (in `MandirMusic/core/mongo.py`) for all database interactions. This ensures that saving and retrieving data (like broadcast lists, banned users, and chat settings) does not block the main event loop, keeping the bot fast and responsive.
